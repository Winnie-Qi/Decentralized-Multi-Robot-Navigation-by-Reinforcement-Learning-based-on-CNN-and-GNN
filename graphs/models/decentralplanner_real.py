...
class DecentralPlannerNet(nn.Module):
    ...
    def forward(self, inputTensor, count, next):
        ...
        # states = torch.zeros(64,10,128)
        # batch_targetGPU = torch.empty(10,64,5)
        for id_agent in range(self.numAgents):
            # DCP_nonGCN
            ...
            if id_agent == 0:
                states = sharedFeature_currentAgent.view(sharedFeature_currentAgent.size(0), -1).unsqueeze(1) # (64,128)
                # sharedFeatureFlatten = sharedFeature_currentAgent.view(sharedFeature_currentAgent.size(0), -1) # (64,128)
            else:
                sharedFeature_currentAgent = sharedFeature_currentAgent.view(sharedFeature_currentAgent.size(0), -1).unsqueeze(1)
                states = torch.cat([states, sharedFeature_currentAgent], dim = 1)


        if next == False:
            action_predict = self.dqn(states) # (64,10,5)
            action_predict = action_predict.permute(1,0,2) # (10,64,5)
        # else:
        #     action_predict = self.target_q_net(states)
        #     action_predict = action_predict.permute(1,0,2)
        #
