...
class multiRobotSim:
    ...
    def setup(self, loadInput, loadTarget, makespanTarget, tensor_map, ID_dataset):
        ...
        for id_agent in range(self.config.num_agents):
            ...
            actionVec_target_CurrentAgents = self.List_MultiAgent_ActionVec_target[id_agent, :, :] # (27,5)
            actionKeyList_target_CurrentAgents = torch.max(actionVec_target_CurrentAgents, 1)[1] # (27)
    ...
    def getNextState(self,posStart,action):

        next_pos = torch.zeros_like(posStart) # (64,2)

        for i in range(posStart.shape[0]):
            next_pos[i,:] = torch.add(posStart[i,:], self.delta[action[i]])
        return next_pos

    def getNextInput(self,map,goal, next_states):
        next_input = torch.zeros(64,10,3,11,11)
        for i in range(next_states.shape[0]):
            self.AgentState.setmap(map[i])
            currentState = self.AgentState.toInputTensor(goal[i], next_states[i]) # (10,3,11,11)
            next_input[i] = currentState
